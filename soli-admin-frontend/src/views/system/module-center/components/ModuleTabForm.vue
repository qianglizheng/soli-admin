...
export interface ModuleTabFormModel {
  id?: number;
  note?: string;
  sort?: number;
  tabCode?: string;
...
const emit = defineEmits<{
  ...
  (e: 'submit', value: Partial<ModuleTabFormModel>): void;
  (e: 'cancel'): void;
}>();
...
function createDefaultForm(): Partial<ModuleTabFormModel> {
  return {
    note: '',
    sort: 1,
    tabCode: '',
...
const dialogTitle = computed(() => props.title || (props.mode === 'create' ? '新增 Tab' : '编辑 Tab'));
